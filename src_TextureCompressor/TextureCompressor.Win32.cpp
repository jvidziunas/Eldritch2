...
static bool Utf8FromWideString(Utf8Char (&out)[bufferSize], const PlatformChar* source) {
	return 0 != WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, source, -1, out, bufferSize, NULL, NULL);
}
...
int wmain(int argc, PlatformChar** argv) {
	...
	return TextureCompressor().Run(convertedArgs.Begin(), convertedArgs.End());
}
