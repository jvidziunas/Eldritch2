		...
		api.CreateClass<Vector>( ET_BUILTIN_WREN_MODULE_NAME( Math ), "Vector",
			{/* Constructors */
				DefineConstructor<Vector ( double, double, double )>( "new", [] ( WrenVM* vm ) {
					SetReturn<Vector>( vm, static_cast<float32>(wrenGetSlotDouble( vm, 1 )), static_cast<float32>(wrenGetSlotDouble( vm, 2 )), static_cast<float32>(wrenGetSlotDouble( vm, 3 )), 0.0f );
				} ),
				DefineConstructor<Vector ( double )>( "splat", [] ( WrenVM* vm ) {
					SetReturn<Vector>( vm, static_cast<float32>(wrenGetSlotDouble( vm, 1 )), static_cast<float32>(wrenGetSlotDouble( vm, 1 )), static_cast<float32>(wrenGetSlotDouble( vm, 1 )), 0.0f );
				} ),
			...
			{/*	Methods */
				DefineStaticMethod<double ( Vector, Vector )>( "dot", [] ( WrenVM* vm ) {
					wrenSetSlotDouble( vm, 0, DotProduct( GetSlotAs<Vector>( vm, 1 ), GetSlotAs<Vector>( vm, 2 ) ) );
				} ),
				DefineStaticMethod<Vector ( Vector, Vector )>( "cross", [] ( WrenVM* vm ) {
				...
				DefineMethod<const Utf8Char* ( )>( "toString", [] ( WrenVM* vm ) {
					...
					fmt::format_to( string, "[x={}, y={}, z={}]", coefficients[0], coefficients[1], coefficients[2] );
