...
ETForceInlineHint Eldritch2::Input::DeviceId wrenGetSlotDeviceId(WrenVM* vm, int slot) ETNoexceptHint {
		...
		api.DefineClass<ActionSetClient>(ET_BUILTIN_WREN_MODULE_NAME(Input), "ActionSet", // clang-format off
			{ /* Static methods */
				ForeignMethod("new(_)", [](WrenVM* vm) ETNoexceptHint {
					wrenSetReturn<ActionSetClient>(vm, /*classSlot =*/0).BindResources(vm, 1);
				}), },
			{ /* Methods */
				ForeignMethod("acquireLocalDevice(_,_)", [](WrenVM* vm) ETNoexceptHint {
					ActionSetClient& self(wrenGetSlotAs<ActionSetClient>(vm, 0));
					ET_ABORT_WREN_UNLESS(vm, wrenGetSlotAs<DeviceLocator>(vm, 1).TryAcquireDevice(wrenGetSlotDeviceId(vm, 2), self, {}), "Unable to acquire device.");
				}),
				ForeignMethod("acquireLocalDevice(_)", [](WrenVM* vm) ETNoexceptHint {
					ActionSetClient& self(wrenGetSlotAs<ActionSetClient>(vm, 0));
					ET_ABORT_WREN_UNLESS(vm, wrenGetSlotAs<DeviceLocator>(vm, 1).TryAcquireDevice(self, {}), "Unable to acquire device.");
				}),
				ForeignMethod("releaseDevices()", [](WrenVM* vm) ETNoexceptHint {
					wrenGetSlotAs<ActionSetClient>(vm, 0).ReleaseDevices();
				}), }); // clang-format on
	}
