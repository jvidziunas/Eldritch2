...
namespace Eldritch2 { namespace Scripting { namespace Wren {
	...
	ET_IMPLEMENT_WREN_CLASS(ActionSet) { // clang-format off
		api.CreateClass<ActionSet>(ET_BUILTIN_WREN_MODULE_NAME(Input), "ActionSet",
			...
			{ /* Methods */
				ForeignMethod("acquireLocalDevice(_,_)", [](WrenVM* vm) {
					DeviceLocator& locator(GetSlotAs<DeviceLocator>(vm, 1));
					DeviceId  id(AsInt(wrenGetSlotDouble(vm, 2)));

					ET_ABORT_WREN_UNLESS(GetSlotAs<ActionSet>(vm, 0).TryAcquireDevice(locator, id), "Unable to acquire device.");
				}),
