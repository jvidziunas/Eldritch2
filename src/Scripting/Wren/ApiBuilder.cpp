...
namespace Eldritch2 {
	namespace Scripting {
		namespace Wren {
			...
			ForeignMethod::ForeignMethod(Utf8Literal signature, Body body) : body(body) {
				ET_ASSERT(body != nullptr, "Wren foreign method must have a body!");
				CopyString(this->signature, signature);
			}

		// ---------------------------------------------------

			ApiBuilder::ApiBuilder(WrenVM* vm, Context& wren) : _vm(vm), _context(&wren) {
