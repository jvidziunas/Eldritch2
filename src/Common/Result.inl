...
namespace Detail {
	...
	ETInlineHint ResultStorage<T, isTriviallyDestructible>::ResultStorage( std::initializer_list<U> initializerList, Args&&... args ) : errorCode( Error::None ) {
...
// ---------------------------------------------------
	...
	ETInlineHint ResultStorage<T, isTriviallyDestructible>::ResultStorage( Args&&... args ) : errorCode( Error::None ) {
...
// ---------------------------------------------------
	...
	template <typename T, bool isTriviallyDestructible>
	ETInlineHint ResultStorage<T, isTriviallyDestructible>::ResultStorage( ResultStorage<T, isTriviallyDestructible>&& other ) : errorCode( other.errorCode ) {
		if (Failed( errorCode )) {
			return;
		}
...
// ---------------------------------------------------
	...
	template <typename T>
	template <typename U, typename... Args>
	ETInlineHint ResultStorage<T, true>::ResultStorage( std::initializer_list<U> initializerList, Args&&... args ) : errorCode( Error::None ) {
		new(eastl::addressof( storage )) ValueType( initializerList, eastl::forward<Args>( args )... );
	}
...
// ---------------------------------------------------
	...
	template <typename T>
	template <typename... Args>
	ETInlineHint ResultStorage<T, true>::ResultStorage( Args&&... args ) : errorCode( Error::None ) {
		new(eastl::addressof( storage )) ValueType( eastl::forward<Args>( args )... );
	}
...
// ---------------------------------------------------
	...
	ETInlineHint Result<CreatedObject>::Result( std::initializer_list<U> initializerList, Args&&... args ) : Detail::ResultStorage<CreatedObject>( initializerList, eastl::forward<Args>( args )... ) {}
	
// ---------------------------------------------------

	template <typename CreatedObject>
	template <typename... Args, typename /*SFINAE*/>
	ETInlineHint Result<CreatedObject>::Result( Args&&... args ) : Detail::ResultStorage<CreatedObject>( eastl::forward<Args>( args )... ) {}

// ---------------------------------------------------

	template <typename CreatedObject>
	ETInlineHint Result<CreatedObject>::Result( ErrorCode error ) : Detail::ResultStorage<CreatedObject>( error ) {}

// ---------------------------------------------------

	template <typename CreatedObject>
	ETInlineHint Result<CreatedObject>::Result( Result<CreatedObject>&& other ) : Detail::ResultStorage<CreatedObject>( eastl::forward<Detail::ResultStorage<CreatedObject>>( other ) ) {}
