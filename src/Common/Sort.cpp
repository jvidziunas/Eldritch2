...
namespace {
	...
	ETForceInlineHint ETPureFunctionHint __m128i ETSimdCall _mm_reverse_epi32(__m128i x) ETNoexceptHint {
		return _mm_shuffle_epi32(x, ReverseMask);
	}

	// ---------------------------------------------------

	ETForceInlineHint ETPureFunctionHint __m128i ETSimdCall _mm_unstripehi_epi32(__m128i x, __m128i y) ETNoexceptHint {
	...
	ETForceInlineHint ETPureFunctionHint __m128i ETSimdCall _mm_unstripelo_epi32(__m128i x, __m128i y) ETNoexceptHint {
		//	NOTE: OLDER MODEL INTEL PROCESSORS (pre-Sandy Bridge) INCUR A PENALTY TO LATENCY WHEN SHIFTING FROM FP -> INTEGER SIMD UNIT
	...
	ETForceInlineHint ETPureFunctionHint __m128i ETSimdCall _mm_stripehi_epi32(__m128i x, __m128i y) ETNoexceptHint {
		//	NOTE: OLDER MODEL INTEL PROCESSORS (pre-Sandy Bridge) INCUR A PENALTY TO LATENCY WHEN SHIFTING FROM FP -> INTEGER SIMD UNIT
		return _mm_shuffle_epi32(_mm_castps_si128(_mm_shuffle_ps(_mm_castsi128_ps(x), _mm_castsi128_ps(y), HighWordsMask)), StripeHiLoMask);
	}
	...
	ETForceInlineHint ETPureFunctionHint __m128i ETSimdCall _mm_stripelo_epi32(__m128i x, __m128i y) ETNoexceptHint {
		//	NOTE: OLDER MODEL INTEL PROCESSORS (pre-Sandy Bridge) INCUR A PENALTY TO LATENCY WHEN SHIFTING FROM FP -> INTEGER SIMD UNIT
		return _mm_shuffle_epi32(_mm_castps_si128(_mm_shuffle_ps(_mm_castsi128_ps(x), _mm_castsi128_ps(y), LowWordsMask)), StripeHiLoMask);
	}
...
void Sse41MergeSort(__m128i* list0, __m128i* list0End, __m128i* list1, __m128i* out) ETNoexceptHint {
	__m128i keysA   = _mm_unstripehi_epi32(_mm_load_si128(list0), _mm_load_si128(list0 + 1));
	__m128i valuesA = _mm_unstripelo_epi32(_mm_load_si128(list0), _mm_load_si128(list0 + 1));

	for (; list0 != list0End; list0 += 2, list1 += 2, out += 2) {
		__m128i keysB   = _mm_unstripehi_epi32(_mm_load_si128(list1), _mm_load_si128(list1 + 1));
		__m128i valuesB = _mm_unstripelo_epi32(_mm_load_si128(list1), _mm_load_si128(list1 + 1));

		//	Reverse elements of B.
		keysB   = _mm_reverse_epi32(keysB);
		valuesB = _mm_reverse_epi32(valuesB);

		__m128i minKeys   = _mm_min_epu32(keysA, keysB);
		__m128i maxKeys   = _mm_max_epu32(keysA, keysB);
		__m128i minValues = _mm_blendv_epi8(valuesA, valuesB, _mm_cmpeq_epi32(keysA, minKeys));
		__m128i maxValues = _mm_blendv_epi8(valuesB, valuesA, _mm_cmpeq_epi32(keysA, minKeys));
