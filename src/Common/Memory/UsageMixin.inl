...
namespace Eldritch2 {
	...
	ETInlineHint UsageMixin<BackingAllocator>::UsageMixin( const UsageMixin<BackingAllocator>& other ) : BackingAllocator( other ), _usage( 0u ), _peakUsage( 0u ) {}

// ---------------------------------------------------

	template <class BackingAllocator>
	template <typename... Arguments>
	ETInlineHint UsageMixin<BackingAllocator>::UsageMixin( Arguments&&... args ) : BackingAllocator( eastl::forward<Arguments>( args )... ), _usage( 0u ), _peakUsage( 0u ) {}
...
// ---------------------------------------------------
	...
	ETInlineHint ETRestrictHint void* UsageMixin<BackingAllocator>::Allocate( typename BackingAllocator::SizeType sizeInBytes, typename BackingAllocator::SizeType alignmentInBytes, typename BackingAllocator::SizeType offsetInBytes, AllocationDuration duration ) {
		const auto	result( BackingAllocator::Allocate( sizeInBytes, alignmentInBytes, offsetInBytes, duration ) );
...
// ---------------------------------------------------
	...
	ETInlineHint ETRestrictHint void* UsageMixin<BackingAllocator>::Allocate( typename BackingAllocator::SizeType sizeInBytes, AllocationDuration duration ) {
		const auto	result( BackingAllocator::Allocate( sizeInBytes, duration ) );
