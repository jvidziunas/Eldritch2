...
// ---------------------------------------------------
	...
	Window::Window() : _window( nullptr ), _messageThread( nullptr ) {}

// ---------------------------------------------------

	Window::Window( Window&& window ) : _window( eastl::exchange( window._window, nullptr ) ), _messageThread( eastl::exchange( window._messageThread, nullptr ) ) {}
...
// ---------------------------------------------------
	...
	// ---
		...
		const HWND	parentWindow( FullscreenMode::Windowed == mode.mode ? nullptr : HWND_TOPMOST );
		const DWORD	style( FullscreenMode::Windowed == mode.mode ? WindowedStyle : FullscreenStyle );
		const UINT	behaviors( FullscreenMode::Windowed == mode.mode ? WindowedBehaviors : FullscreenBehaviors );
		RECT		clientArea;

		clientArea.left		= 0;
		clientArea.top		= 0;
		clientArea.right	= FullscreenMode::Windowed != mode.mode ? GetSystemMetrics( SM_CXFULLSCREEN ) : mode.widthInPixels;
		clientArea.bottom	= FullscreenMode::Windowed != mode.mode ? GetSystemMetrics( SM_CYFULLSCREEN ) : mode.heightInPixels;
...
// ---------------------------------------------------
	...
	void Swap( Window& window0, Window& window1 ) {
		...
		Swap( window0._mode,			window1._mode );
		Swap( window0._window,			window1._window );
		Swap( window0._messageThread,	window1._messageThread );
	}
