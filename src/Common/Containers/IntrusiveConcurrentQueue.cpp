...
namespace {
	...
	template <typename T>
	ETInlineHint ETPureFunctionHint T AtomicLoad( T& value, std::memory_order order ) {
...
// ---------------------------------------------------
	...
	template <typename T>
	ETInlineHint ETPureFunctionHint void AtomicStore( T& value, T desiredValue, std::memory_order order ) {
		return std::atomic_store_explicit( reinterpret_cast<std::atomic<T>*>(&value), desiredValue, order );
	}

}	// anonymous namespace

	QueueStorage::QueueStorage() : _head( &_stub ), _tail( &_stub ) {
