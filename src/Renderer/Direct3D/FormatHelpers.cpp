...
namespace Direct3D {
	...
	ETPureFunctionHint uint32 BitsPerPixel( DXGI_FORMAT fmt ) {
		switch( fmt ) {
...
// ---------------------------------------------------
	...
	ETPureFunctionHint void GetSurfaceInfo( uint32 width, uint32 height, DXGI_FORMAT fmt, uint32& numBytes, uint32& rowBytes, uint32& numRows ) {
...
// ---------------------------------------------------
	...
	ETPureFunctionHint DXGI_FORMAT BuildDxgiFormat( const GpuLayout layout, const GpuStorageFormat format, const FilteringFlags flags ) {
		struct FormatData {
			struct srgbData {
