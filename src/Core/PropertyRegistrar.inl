...
namespace Eldritch2 { namespace Core {
	namespace Detail {
		...
		struct NumericParser {
			...
			static ETInlineHint ETPureFunctionHint void Parse(StringView string, Value& value) ETNoexceptHint {
	...
	PropertyRegistrar& PropertyRegistrar::DefineProperty(StringView name, PodValue& property) {
		using ParserType = Detail::NumericParser<eastl::is_floating_point<PodValue>::value, eastl::is_signed<PodValue>::value>;

		return DefineProperty(name, [&property](StringView value) ETNoexceptHint {
			ParserType::Parse(value, property);
		});
	}
