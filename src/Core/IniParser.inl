...
namespace {
	...
	static ETPureFunctionHint const Utf8Char* StripByteOrderMark( const Utf8Char* const begin, const Utf8Char* const end ) {
...
// ---------------------------------------------------
	...
	//	Consume the BOM, if one exists.
		const Utf8Char*	line( StripByteOrderMark( source.Begin(), source.End() ) );
	...
	//	Scan through each line in the range.
		for (const Utf8Char* readEnd( Find( line, source.End(), '\n' ) ); source.ContainsIterator( line ); (line = readEnd), readEnd = Find( line, source.End(), '\n' )) {
