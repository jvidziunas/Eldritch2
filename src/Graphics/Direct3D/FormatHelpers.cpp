...
// INCLUDES
//==================================================================//
#include <Graphics/Direct3D/FormatHelpers.hpp>
//------------------------------------------------------------------//
#include <DXGIFormat.h>
//------------------------------------------------------------------//

namespace Eldritch2 { namespace Graphics { namespace Direct3D {
	...
	ETPureFunctionHint void GetSurfaceInfo(uint32 width, uint32 height, DXGI_FORMAT format, uint32& numBytes, uint32& rowBytes, uint32& numRows) {
		...
		if (blockCompressed) {
			...
			const uint32 bitsPerPixel(GetBitsPerPixel(format));
	...
	ETPureFunctionHint DXGI_FORMAT GetDxgiFormat(GpuBitLayout layout, GpuStorageFormat format, GpuFilteringFlags flags) {
		...
		static const FormatData textureLayouts[GpuBitLayout::Count] = {
			{ { // _32_32_32_32
		...
		if ((layout < GpuBitLayout::Count) & (format < GpuStorageFormat::Count)) {
			return textureLayouts[static_cast<size_t>(layout)].formats[static_cast<size_t>(format)].srgbFormats[static_cast<bool>(flags & GpuFilteringFlags::SampleAsSrgb)];
		}
...
}}} // namespace Eldritch2::Graphics::Direct3D
