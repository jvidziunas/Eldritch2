		...
		executor.AwaitWork(
			...
			[this](JobExecutor& executor) {
				for (GeometryConcept& concept : _geometryConcepts) {
					if (concept.ShouldRebuildHierarchy()) {
						ET_PROFILE_SCOPE("Graphics/AccelerationStructures", "Build Geometry Hierarchy", 0xFFAAFF);
			...
			[this](JobExecutor& executor) {
				for (LightConcept& concept : _lightConcepts) {
					if (concept.ShouldRebuildHierarchy()) {
						ET_PROFILE_SCOPE("Graphics/AccelerationStructures", "Build Light Hierarchy", 0xFFAAFF);
