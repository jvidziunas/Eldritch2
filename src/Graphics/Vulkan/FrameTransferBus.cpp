...
namespace Eldritch2 { namespace Graphics { namespace Vulkan {
	...
	FrameTransferBus::FrameTransferBus(FrameTransferBus&& bus) :
		FrameTransferBus() {
		Swap(*this, bus);
	}
	...
	void FrameTransferBus::SendToGpu(VkImage target, VkOffset3D targetOffset, VkExtent3D targetExtent, VkImageSubresourceLayers subresource) {
	...
	void FrameTransferBus::SendToGpu(VkBuffer target, VkDeviceSize targetOffset, VkDeviceSize targetExtent) {
		VkBufferCopy copy {
	...
	void FrameTransferBus::SendToHost(VkImage source, VkOffset3D sourceOffset, VkExtent3D sourceExtent, VkImageSubresourceLayers subresource) {
		VkBufferImageCopy copy {
			/*bufferOffset =*/0u, // Offset in staging buffer will be filled in below.
			TightPack,
			TightPack,
	...
	void FrameTransferBus::SendToHost(VkBuffer source, VkDeviceSize sourceOffset, VkDeviceSize sourceExtent) {
		VkBufferCopy copy {
			sourceOffset,
	...
	void FrameTransferBus::BindRegion(VkImage target, const VkSparseImageMemoryBind& bind) {
		_pendingImageBinds.EmplaceBack(target, bind);
	}

	// ---------------------------------------------------

	void FrameTransferBus::BindRegion(VkImage target, const VkSparseMemoryBind& bind) {
		_pendingImageOpaqueBinds.EmplaceBack(target, bind);
	}

	// ---------------------------------------------------

	bool FrameTransferBus::CheckCommandsConsumed(Gpu& gpu) const {
		return vkGetFenceStatus(gpu, _commandsConsumed) != VK_NOT_READY;
	}

	// ---------------------------------------------------

	VkResult FrameTransferBus::SubmitCommands(Gpu& gpu) {
		...
		ET_FAIL_UNLESS(gpu.BindAsync(QueueConcept::SparseBinding, VK_NULL_HANDLE, {
			AsBindSparseInfo(0u, nullptr, // No buffer binds.
							 ...
							 uint32(imageBinds.GetSize()), imageBinds.GetData(),
	...
	VkResult FrameTransferBus::InsertSignalSemaphore(Gpu& gpu, VkSemaphore& outSemaphore) {
	...
	void FrameTransferBus::EraseSignalSemaphore(Gpu& gpu, VkSemaphore semaphore) {
		{
			Lock _(_signalsMutex);
	...
	VkResult FrameTransferBus::BindResources(Gpu& gpu, VkDeviceSize transferBufferSize) {
	...
	void FrameTransferBus::FreeResources(Gpu& gpu) {
	...
	void Swap(FrameTransferBus& lhs, FrameTransferBus& rhs) {
		using ::Eldritch2::Swap;
